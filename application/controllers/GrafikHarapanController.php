...
class GrafikHarapanController extends CI_Controller
{
	...
	public function index($id1 = NULL, $id2 = NULL)
	{
		...
		(SELECT COUNT(IF(skor_jawaban != 0, 1, NULL))
 		FROM jawaban_pertanyaan_harapan_$table_identity JOIN survey_$table_identity ON jawaban_pertanyaan_harapan_$table_identity.id_responden = survey_$table_identity.id_responden WHERE jawaban_pertanyaan_harapan_$table_identity.id_pertanyaan_unsur = nilai_tingkat_kepentingan_$table_identity.id_pertanyaan_unsur_pelayanan && is_submit = 1) AS jumlah_pengisi
