...
class PertanyaanUnsurSurveiController extends CI_Controller
{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $value) {
			...
			// $row[] = '<b>' . $value->nomor_unsur . '. ' . $value->nama_unsur_pelayanan . ' ' . $required . '</b>
			// <button type="button" class="btn btn-link" data-toggle="modal" data-target="#edit' . $value->id_unsur . '" title="Ubah nomor unsur dan nama unsur">
	...
	public function change_list()
	{
		...
		// $data = $this->input->post('positions');
		// var_dump($data);
		
		foreach($this->input->post('positions') as $position) {
			$index = $position[0];
			$newPosition = 'U' . $position[1];
			// var_dump($index);
			// var_dump($newPosition);

			$this->db->query("UPDATE unsur_pelayanan_cst427 SET nomor_unsur = '$newPosition' WHERE id = $index");
		}
